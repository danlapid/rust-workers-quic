...
| --- | --- | --- | --- | --- |
| emscripten frontend | `guybedford/emscripten` branch `cf` | `08a1fc28569c93a86f9e737bac508332a298615e` (`6.0.3-git`) | Direct clone to `.work/emscripten` | Integrated epoll, async DNS, UDP/TCP over `-sNODERAWSOCKETS`, wasm-bindgen, and blocking socket operations. `patches/emscripten-recvmmsg.patch` adds batched receives over the existing `recvmsg` backend. |
